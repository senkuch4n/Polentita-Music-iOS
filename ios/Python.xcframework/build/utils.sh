...
install_dylib () {
    ...
    # When the build has no signing identity (CODE_SIGNING_ALLOWED=NO, e.g. an
    # unsigned .ipa for AltStore / Sideloadly, which re-sign on install), fall
    # back to ad-hoc signing so `set -e` doesn't abort on "no identity found".
    if [ -z "$EXPANDED_CODE_SIGN_IDENTITY" ] || [ "$EXPANDED_CODE_SIGN_IDENTITY" = "-" ]; then
        echo "Signing framework ad-hoc (no signing identity)..."
        /usr/bin/codesign --force --sign - "$CODESIGNING_FOLDER_PATH/$FRAMEWORK_FOLDER"
    else
